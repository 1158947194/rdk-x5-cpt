	...
	for (i = 0; Chinese[i] != '\0'; i ++)		//遍历汉字串
	{
		...
		if (pChinese >= OLED_CHN_CHAR_WIDTH)
		{
			...
			else if(CF_Size == OLED_CF32X32)
			{
				...
				/*将汉字字模库OLED_CF32x32的指定数据以32*32的图像格式显示*/
				OLED_ShowImage(X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 32, Y, 32, 32, OLED_CF32x32[pIndex].Data);				
			}
			else if(CF_Size == OLED_CF16X32)
			{
				for (pIndex = 0; strcmp(OLED_CF16x32[pIndex].Index, "") != 0; pIndex ++)
				{
					/*找到匹配的汉字*/
					if (strcmp(OLED_CF16x32[pIndex].Index, SingleChinese) == 0)
					{
						break;		//跳出循环，此时pIndex的值为指定汉字的索引
					}
				}
				
				/*将汉字字模库OLED_CF16x32的指定数据以16*32的图像格式显示*/
				OLED_ShowImage(X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, Y, 16, 32, OLED_CF16x32[pIndex].Data);				
			}
		}
	}
}
